...
public class KeyManagerInitializer extends LicenseManagerStageInitializer {
    private static final Logger LOGGER = LogManager.getLogger(KeyManagerInitializer.class);

    public KeyManagerInitializer(final Controller controller) {
        super(JfxResource.KEY_MANAGER, JfxResource.KEY_MANAGER, controller, true);
    }

    public KeyManagerInitializer(final String name, final String stageKey, final Controller controller, boolean modal, final Locale locale) {
    ...
    public void stageInit() {
        getStage().setTitle(ProgramSettingsService.getInstance().getSettings().getProgramTitleBarPrefix()
                + getResources().getString("ui.keymanager.title"));
